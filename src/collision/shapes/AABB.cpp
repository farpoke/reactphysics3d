...
bool AABB::testRayIntersect(const Ray& ray) const {
    ...
    decimal adx = cnl::abs(d.x);
    if (cnl::abs(m.x) > e.x + adx) return false;
    decimal ady = cnl::abs(d.y);
    if (cnl::abs(m.y) > e.y + ady) return false;
    decimal adz = cnl::abs(d.z);
    if (cnl::abs(m.z) > e.z + adz) return false;
    ...
    // separating axis
    if (cnl::abs(m.y * d.z - m.z * d.y) > e.y * adz + e.z * ady) return false;
    if (cnl::abs(m.z * d.x - m.x * d.z) > e.x * adz + e.z * adx) return false;
    if (cnl::abs(m.x * d.y - m.y * d.x) > e.x * ady + e.y * adx) return false;
